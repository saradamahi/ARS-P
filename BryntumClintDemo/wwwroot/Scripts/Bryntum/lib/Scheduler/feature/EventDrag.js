...
export default class EventDrag extends DragBase {
    ...
    isElementDraggable(el, event) {
        ...
        const
            ...
            resizable      = eventResizable &&
                ...
                (((eventResizable === true || eventResizable === 'start') && eventResize.resize.overStartHandle(event, eventElement)) ||
                ((eventResizable === true || eventResizable === 'end') && eventResize.resize.overEndHandle(event, eventElement)));
    ...
    onDragAbort() {
        // Edge case: If this drag was a no-op, and underlying data was changed while drag was ongoing (e.g. web socket
        // push), we need to manually force a view refresh to ensure a correct render state
        if (this.dragData.initialAssignmentsState.find(({ resource }, i) => resource.id !== this.dragData.assignmentRecords[i]?.resourceId)) {
            this.client.refresh();
        }
    ...
    getDragData(info, event) {
        ...
        return ObjectHelper.assign(dragData, {
            ...
            initialAssignmentsState : assignmentRecords.map(as => ({
                startDate : as.event.startDate,
