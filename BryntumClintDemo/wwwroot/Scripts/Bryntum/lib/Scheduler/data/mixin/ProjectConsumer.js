...
export default Target => class ProjectConsumer extends (Target || Base) {
    ...
    static get configurable() {
        return {
             ...
             * Can be omitted in favor of individual store configs or {@link Scheduler.crud.mixin.CrudManagerView#config-crudManager} config.
             *
        ...
        clsProto[`change${storeNameCap}`] = function(store, oldStore) {
            ...
            if (store !== oldStore) {
                ...
                me.projectSubscribers.forEach(subscriber => {
                    if (subscriber[storeName] !== store && subscriber[`attachTo${storeNameCap}`]) {
