...
export default class TimelineBase extends GridBase.mixin(
    ...
    static get defaultConfig() {
        return {
             ...
             * the scroll action, see {Scheduler.view.mixin.TimelineScroll#function-scrollToDate} for reference.
             * @config {Date|Object}
             * @category Common
             */
            visibleDate : null,
    ...
    construct(config = {}) {
        ...
        me.onSchedulerViewportResize = me.throttle(me.onSchedulerViewportResize, me.viewportResizeTimeout);
    ...
    createSubGrid(region, config = {}) {
        ...
        if (region === (me.schedulerRegion || 'normal')) {
            ...
            // Use Scheduler's header
            config.headerClass = Header;
        ...
        // is used to house the verticalTimeAxis, and so it must all be oberflow:visible
    ...
    /**
     * Breaks the link between this Timeline and the passed Timeline set up by the
    ...
    onSchedulerViewportResize(width, height, oldWidth, oldHeight) {
        if (this.isPainted) {
            ...
            if (
                ...
                timeAxisViewModel.availableSpace = subGrid.width - DomHelper.scrollBarWidth;
            }
            else {
                timeAxisViewModel.availableSpace = isHorizontal ? subGrid.width : me._bodyRectangle.height;
            }
    ...
    async waitForTransition() {
        ...
        if (this.isAnimating) {
            await this.await('transitionend');
        }
    }
...
VersionHelper.setVersion('scheduler', '4.1.0');
