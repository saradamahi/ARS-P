...
export default class Store extends Base.mixin(
    ...
    onDataChange({ source : storage, action, added, removed, replaced, oldCount, items, from, to }) {
        ...
        switch (action) {
            ...
            case 'move':
                 ...
                 * @event move
                 * @param {Core.data.Store} source This Store
                 * @param {Core.data.Model} record (DEPRECATED) The first record moved (The {@link Core.data.mixin.StoreCRUD#function-move}
                 * API now accepts an array of records to move).
    ...
    indexOf(recordOrId, visibleRecords = false) {
        // Only check records actually in the store
        if (recordOrId?.isModel && !recordOrId.stores.includes(this)) {
            return -1;
        }
     ...
     * @category Iteration
     */
    forEach(fn, thisObj = this, includeFilteredOutRecords = false) {
        const callback = (r, i) => {
        ...
        if (this.isTree) {
            this.rootNode.traverseWhile(callback, false, includeFilteredOutRecords);
        }
        else {
            // native forEach cannot be aborted by returning false, have to loop "manually"
            const records = includeFilteredOutRecords ? this.storage.allValues : this.storage.values;
     ...
     * @param {Boolean} [includeFilteredOutRecords=false] Pass true to traverse filtered out nodes in a tree
     * @category Traverse
     */
    traverse(fn, topNode = this.rootNode, skipTopNode = topNode === this.rootNode, includeFilteredOutRecords = false) {
        const
            me          = this,
            { storage } = me;
        ...
        if (me.isTree) {
            ...
            topNode.traverse(fn, skipTopNode, includeFilteredOutRecords);
        }
        else {
            const records = includeFilteredOutRecords ? storage.allValues : storage.values;

            for (let i = 0, { length } = records; i < length; i++) {
                records[i].traverse(fn, false, includeFilteredOutRecords);
            }
        }
    }
