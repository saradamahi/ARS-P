...
export default class DomHelper {
     ...
     * @internal
     */
    static normalizeChildren(domConfig, namedChildren) {
        ...
        if (children && !(domConfig instanceof Node)) {
            if (Array.isArray(children)) {
                for (i = 0; i < children.length; ++i) {
                    DomHelper.normalizeChildren(children[i], namedChildren);
                }
            }
            else {
                kids = children;
                ...
                for (name in kids) {
                    ...
                    if (child) {
                        if (!(child instanceof Node)) {
                            ...
                            DomHelper.normalizeChildren(child, namedChildren);
                        }
    ...
    static createElement(config = {}, options, ...args) {
        let returnAll = options,
            element, i, ignoreRefs, key, name, value, refOwner, refs, syncIdField;
