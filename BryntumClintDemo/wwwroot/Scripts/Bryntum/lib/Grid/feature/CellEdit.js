...
export default class CellEdit extends Delayable(InstancePlugin) {
    ...
    cancelEditing(silent = false, triggeredByEvent) {
        ...
        if (editorContext) {
            ...
            if (editor.containsFocus) {
                ...
                if (editor.inputField.owns(DomHelper.getActiveElement(grid))) {
                    DomHelper.focusWithoutScrolling(grid.focusElement);
                }
    ...
    async finishEditing() {
        ...
        if (editorContext) {
            ...
            await me.grid.waitForTransition?.();
    ...
    onEditorFocusOut(event) {
        ...
        if (me.editorContext && !me.editor.isFinishing && me.editor.inputField.owns(event._target) &&
            (event.toWidget !== grid || grid.isLocationEqual(grid.focusedCell, me.editorContext.selector))) {
