...
export default class Row extends Delayable(Base) {
    ...
    renderCell({
        ...
        // `shouldSetContent` falsy value says that content is already set by the column (i.e. Widget column).
        // `frameworkSetsContent` truthy value says that content is going to be set by framework renderer.
        // We should dive into regular cell content setting routing only if first is truthy and second is falsy.
        let frameworkSetsContent = false;

        // Ask wrapper if there is a framework renderer, for example React JSX
        if (grid.hasFrameworkRenderer) {
            frameworkSetsContent = grid.hasFrameworkRenderer({
                cellContent,
                rendererData,
                record
            });
        ...
        if (shouldSetContent && !frameworkSetsContent) {
        ...
        // processTreeContent is implemented in the framework wrapper (if any)
        // Let the wrapper to render the framework component in the cell
