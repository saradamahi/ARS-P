...
export default class ScrollManager extends Delayable(Base) {
    ...
    startMonitoring(elementOrConfig) {
        ...
        if (element && (element.scrollWidth > element.offsetWidth || verticalElement.scrollHeight > verticalElement.offsetHeight)) {
            ...
            // Each monitor target gets its own handler bound to the config passed, which makes it easy to control the
            // scope
            ...
            const scrollHandler = me.onVerticalElementScroll.bind(config);

            me.mouseWheelDetacher = EventHelper.on({
     ...
     * @param {HTMLElement} [element] Element for which monitoring is not desired any more and should stop as soon as possible.
     * Preferably right away, but no later than on next frame :)
     * @internal
     */
    stopMonitoring(element) {
        ...
        if (!element) {
            me.monitoring.forEach(cfg => me.stopMonitoring(cfg.element));
            return;
        }

        const
            config = me.monitoring.find(cfg => cfg.element === element);

        // cant stop nothing...
        if (!config) return;

        me.stopScroll(config);
        me.mouseMoveDetacher?.();
        me.mouseWheelDetacher?.();
    ...
    onVerticalElementScroll() {
        const
            config   = this,
            vertical = config.verticalElement;

        // Ignore scroll if it was triggered by ScrollManager
        if (config.callback && !config.scrolling) {
            config.scrollRelativeTop = vertical.scrollTop - config.startScrollTop;
