...
export default Target => class GridSelection extends (Target || Base) {
    ...
    selectAll() {
        ...
        if (me.store.isFiltered && !me.selectionMode.deselectFilteredOutRecords) {
            ...
            me.selectedRecords = me.store.allRecords.filter(r => !r.isSpecialRow);
        }
    }
     ...
     * @param {Boolean} [keepFilteredOutSelection] Pass `false` to clear all selected records, and `true` to only
     * clear selected records in the current filtered set of records
     * @category Selection
     */
    deselectAll(keepFilteredOutSelection = false) {
        const me = this;

        if (keepFilteredOutSelection) {
