...
var _bryntum$schedulerpro = bryntum.schedulerpro,
    LocaleManager = _bryntum$schedulerpro.LocaleManager,
    Localizable = _bryntum$schedulerpro.Localizable,
    AsyncHelper = _bryntum$schedulerpro.AsyncHelper,
    AjaxHelper = _bryntum$schedulerpro.AjaxHelper,
    DomHelper = _bryntum$schedulerpro.DomHelper,
    Button = _bryntum$schedulerpro.Button,
    Popup = _bryntum$schedulerpro.Popup,
    Tooltip = _bryntum$schedulerpro.Tooltip,
    Toast = _bryntum$schedulerpro.Toast,
    Widget = _bryntum$schedulerpro.Widget,
    BrowserHelper = _bryntum$schedulerpro.BrowserHelper,
    EventHelper = _bryntum$schedulerpro.EventHelper,
    ResizeHelper = _bryntum$schedulerpro.ResizeHelper,
    VersionHelper = _bryntum$schedulerpro.VersionHelper,
    DataGenerator = _bryntum$schedulerpro.DataGenerator,
    Fullscreen = _bryntum$schedulerpro.Fullscreen,
    Events = _bryntum$schedulerpro.Events,
    GlobalEvents = _bryntum$schedulerpro.GlobalEvents,
    Rectangle = _bryntum$schedulerpro.Rectangle;
"use strict";

var _window$requireModern, _window;
...
(_window$requireModern = (_window = window).requireModernBrowser) === null || _window$requireModern === void 0 ? void 0 : _window$requireModern.call(_window, window.bryntum.product.fullName); // Load product config
// Load localization
...
var product = window.bryntum.product,
...
var Shared = /*#__PURE__*/function (_Localizable) {
  ...
  _createClass(Shared, [{
    ...
    value: function insertHeader() {
      ...
      var tools = document.getElementById('tools'),
        ...
        onClick: function onClick(_ref) {
          ...
          if (window.grid || window.scheduler || window.gantt || window.calendar) {
    ...
    value: function startRootCause() {
      ...
      var isBugBash = Boolean(BrowserHelper.getCookie('bugbash')),
        ...
        gantt: '9df03cbdc00b455de8bfe58d831e6fd76cc8881e'
          ...
          appId = appIds[isBugBash ? 'bugbash' : product.name] || 'unknown',
          version = VersionHelper.getVersion(product.name),
    ...
    value: function performVersionCheck() {
      ...
      AjaxHelper.get("https://bryntum.com/latest/?product=".concat(product.onlineId), {
        parseJson: true,
        credentials: 'omit'
    ...
    value: function notifyIfLaterVersionExists(response) {
      ...
      if (latestVersion && VersionHelper.checkVersion(product.name, latestVersion, '<')) {
    ...
    while (1) {
      switch (_context6.prev = _context6.next) {
        case 0:
          ...
          window.calendar = bryntum.query('calendar', true); // Show code editor
